...
따라서 상위 모듈이나 하위 모듈의 역활을 하는 가상의 모듈을 만들어 사용해야 한다. `테스트 드라이버(test driver)`는 테스트 모듈을 호출하여 실행 결과를 받는 상위 가상 모듈이며 `테스트 스텁(test stub)`은 테스트 모듈이 호출하는 하위 가상 모듈이다.
...
테스트 코드들은 크게보면 필요한 페이지를 만들고(Build), request를 요청하여 반응한 xml 결과가 생기고(Operate), 그 결과가 어떤지 검사(Check)하는 일관된 과정을 거치지만 각각의 단계에 대한 상세내용이 너무 많이 들어나 이해하기 어려우며 비슷한 과정을 반복하는 중복코드가 많아 양은 많지만 표현력이 떨어지는 테스트 코드다. 
...
개선된 코드에서는 제품 코드를 위한 API를 그대로 테스트 코드에 사용하지 않고 제품 코드용 API를 사용해서 만든 테스트 코드용 API를 사용한다. 테스트 코드용 API를 사용하면 불필요한 상세내용이 들어나는 것을 방지하고 중복코드를 줄여 테스트 코드 작성을 더 쉽게 할 수 있을뿐만 아니라 가독성도 높힐 수 있다. 
...
[좋은단위테스트 - ](https://www.slideshare.net/OnGameServer/2-1-8104721)  
[구글테스트](https://www.slideshare.net/jinhwason/ss-69528881)  

# 테스트 가능한 코드

# Gmock
[How to use Google Mock - 놀고 먹고 일하는 이야기 블로그](https://ungodly-hour.tistory.com/17)  
[mock 객체란?](https://happy-coding-day.tistory.com/entry/Mock-%EA%B0%9D%EC%B2%B4%EB%9E%80-%EB%AC%B4%EC%97%87%EC%9D%BC%EA%B9%8C-%EC%99%9C-%EC%8D%A8%EC%95%BC%EB%90%A0%EA%B9%8C)  
