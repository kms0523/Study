...
$$ \begin{gathered} \mathbf K^i  \mathbf{\hat{d}^i} = \mathbf r^i  \\\\ \text{Where,} \quad \mathbf K^i = \int_{\Omega_i} \mathbf {B^TEB} dV, \quad \mathbf r_e = \int_{\partial\Omega_i} \mathbf N^T \mathbf b dV + \int_{\partial\Omega_i} \mathbf N^T \mathbf t dS \end{gathered}  $$
...
# 봉 요소
`봉 요소(bar element)`는 1차원 요소로 `축방향(axial)` 하중만 작용하는 경우이다.
