...
이 떄, inner product의 4번 성질에 의해 $v = 0_V$이다. $\quad\tiny\blacksquare$
...
따라서, $v_g$가 존재한다면 반드시 $v_g=\overline{g(\beta_i)}\beta_i$ 형태로 유일하게 결정된다. $\quad\tiny\blacksquare$
...
즉, 임의의 $g$에 대해 $f_{v_g} = g$가 되는 $v_g$는 반드시 하나 이상 존재한다. $\quad\tiny\blacksquare$
...
### 명제2(Schur's theorem)
$n$차원 inner product space $V / \mathbb F$와 $T \in \End(V)$가 있다고 하자.

$T$의 characteristic polynomial를 $\varphi_T(\lambda)$라 할 떄, 다음을 증명하여라.
$$ \varphi_T(\lambda) \text{is split} \implies \exist \text{ orthonormal basis } \beta \quad s.t. \quad \frak m_\beta^\beta(T) \text{ is an upper triangular matrix} $$
...
보조명제2.1에 의해 $\span(v)^\perp$는 split됨으로, 귀납적 가정에 의해 다음이 성립한다.
$$ \exist \text{orthonormal basis } \gamma = \{\gamma_1, \cdots, \gamma_{n-1} \} \quad s.t. \quad \frak m_{\gamma}^{\gamma}(T|_{\span(v)^\perp}) \text{ be an upper triangular matrix.} $$
...
동시에 $\beta$를 이루고 있는 $\gamma$와 $v$는 direct sum 관계에 있는 두 공간의 기저임으로 $\beta$는 $V$의 orthonormal basis가 된다.$\quad\tiny\blacksquare$
...
보조명제2.1.1에 의해서 $\span(v)^\perp$는 $T$ invariant임으로 다음이 성립한다.
$$ \begin{aligned} \frak m_{\beta}^{\beta}(T) &= \begin{bmatrix} \frak{m}_\beta(T(\gamma_1)) & \cdots & \frak{m}_\beta(T(\gamma_{n-1})) & \frak{m}_\beta(T(v)) \end{bmatrix} \\&= \begin{bmatrix} \begin{array}{c | c} \begin{array}{} \\ \frak m_\gamma^\gamma(T|_{\span(v)^\perp}) \\  \\ \hline 0 \end{array} & \begin{array}{} a_1 \\ \vdots \\ a_n \end{array} \end{array} \end{bmatrix} \end{aligned} $$
...
그럼으로, $T|_{\span(v)^\perp}$는 split 된다. $\quad\tiny\blacksquare$
...
$$ B(T(x),v) = B(x, T^*(v)) = B(x, \overline\lambda v) = \lambda B(x, v) = 0_\mathbb F $$

임의의 $x \in \span(v)^\perp$에 대해, $T(x)$와 $v$가 수직함으로, $T(x) \in \span(v)^\perp$이다.

따라서, $\span(v)^\perp$는 $T \text{ invariant}$이다. $\quad\tiny\blacksquare$
...
즉, $\frak m_\beta^\beta(L_A) \sim A$이다. $\quad\tiny\blacksquare$
...
norm의 정의에 의해, 모든 $j$에 대해 $a^j = 0$임을 알 수 있다. 따라서 linearly independent하다. $\quad\tiny\blacksquare$
...
$$ y = \frac{B(y,s_i)}{B(s_i, s_i)} s_i \quad\tiny\blacksquare$$
...
따라서, $aw_1 + w_2 \in W^\perp$임으로 연산에 닫혀있다. $\quad\tiny\blacksquare$
...
따라서, $v-u \in W^\perp$임으로, $V = W + W^\perp$이다. $\quad\tiny\blacksquare$
...
내적의 정의에 의해 $w = 0_V$이다. $\quad\tiny\blacksquare$
