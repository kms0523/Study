...
두 점 $\bm X, \bm X + \Delta \bm X$와 변위장 $\mathbf d$가 있을 때, $t$일 때, 두 점의 공간좌표는 다음과 같다.
...
이 때, $\Delta \bm X$가 충분히 작아 선형으로 근사할 수 있다면 다음과 같다.
$$ \begin{aligned} \Delta x_i &= \Delta \bm X_i + d_i (\bm X + \Delta \bm X, t) - d_i (\mathbf X, t) \\ &= \Delta \bm X_i + \frac{\partial d_i}{\partial \bm X_j} \Delta \bm X \end{aligned} $$
$$ \therefore \Delta \mathbf x = \Delta \bm X + \nabla \mathbf d \cdot \Delta \bm X = \mathbf F \Delta \bm X  $$

이 때, 2차 텐서인 $\Delta \mathbf d$를 displacement gradient, $\mathbf F$를 deformation gradient라고 한다. $\bf F$를 deformation gradient라고 하는 이유는 $\bf F = \nabla p$이기 때문이다.

다음으로, 변형 후 길이가 어떻게 변했는지 보자.
$$ \Delta \mathbf x \cdot \Delta \mathbf x = \Delta \bm X ^T \mathbf F^T \mathbf F \Delta \bm X = \Delta \bm X ^T \mathbf C \Delta \bm X $$

이 떄, $\bf C$를 Chauchy-Green deformation tensor라고 한다. $\bf C$를 풀어쓰면 다음과 같다.
$$ \mathbf C = \mathbf {(I + \nabla d)}^T \mathbf {(I + \nabla d)} = \mathbf {I + (\nabla d)}^T + \nabla \mathbf d + (\nabla \mathbf d)^T \nabla \mathbf d = \mathbf I + 2 \mathbf E_L $$
$$ \text{Where, } \mathbf E_L =\frac{1}{2} \left((\nabla \mathbf d)^T + \nabla \mathbf d + (\nabla \mathbf d)^T \nabla \mathbf d \right) $$

만약 $\bf C = I$면 변형전 길이와 변형후 길이가 같음으로 강체의 운동을 나타내게 된다. 따라서 $\mathbf E_L$이 연속체에서 물질점에 변위가 발생함에 따라 변하는 길이를 표현하는 값이 된다. 이 때, $\mathbf E_L$을 `Lagrange strain tensor` 또는 `finite deformation tensor`라고 한다.

이 때, $\nabla \mathbf d$가 충분히 작아 고차항인 $(\nabla \mathbf d)^T \nabla \mathbf d$를 무시할 수 있다면 다음과 같이 간단해진다.
...
> 참고  
[book] (Lai et al) Introduction to Continuum Mechanics Chapter3.7
