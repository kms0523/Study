...
$$ \mathbf x_n = \mathbf p_n(t), \quad n=1, \cdots, N, \cdots $$
...
이러한 서술 방법을 물질 관점 혹은 Lagrangean 관점이라고 한다.
...
$$ \mathbf Q = \mathbf G( \mathbf f({\bm X}, t), t) = \mathbf F(\bm X, t)$$

## Material Derivative
연속체 한 점의 물리량이 시간에 따라 변하는 정도를 `물질미분(material derivative)`이라고 하며, $D/Dt$로 표현한다.

스칼라 물리량 $\phi$가 다음과 같이 Lagrangean 관점과 Eulerian 관점으로 표현되었다고 하자.
...
이번에는 벡터 물리량 $\boldsymbol \phi$가 다음과 같이 Lagrangean 관점과 Eulerian 관점으로 표현되었다고 하자.
...
## Displacement Field
연속체 한 점의 변위 벡터 $\bf d$는 식(1)에 의해 다음과 같다.
$$ \mathbf d(\bm X, t) = \mathbf p(\bm X, t) - \bm X $$

## Kinematic Equation for Rigid Body Motion
### translation
강체 병진운동의 운동방정식은 다음과 같다.
$$ \mathbf x = \bm X + \mathbf c(t) $$

따라서, 변위 벡터는 다음과 같다.
$$ \mathbf d(\bm X,t) = \mathbf c(t) $$

이는 위치에 상관없이 모든 점이 동일한 크기와 방향의 변위를 갖는다는 것을 의미한다.

### Rotation
강체 회전운동의 운동방정식은 다음과 같다.
$$ \mathbf x - \mathbf b = \mathbf R(t)(\bm X - \mathbf b) $$
$$ \text{Where, } \mathbf b \text{ is rotational center and } \\ \mathbf R \text { is a proper orthogonal tensor} $$

### General motion
강체의 운동방정식은 다음과 같다.
$$ \mathbf x - \mathbf b = \mathbf R(t)(\bm X - \mathbf b) + \mathbf c(t) $$
...
## Deformation
두 점 $\bm X, \bm X + \Delta \bm X$와 변위장 $\mathbf d$가 있을 때, $t$에서 두 점의 공간좌표는 다음과 같다.
$$ \mathbf x = \bm X + \mathbf d (\mathbf X, t) \\ \mathbf x + \Delta \mathbf x = \bm X + \Delta \bm X + \mathbf d (\bm X + \Delta \bm X, t) $$

따라서 $\Delta \mathbf x$는 다음과 같다.
$$ \Delta \mathbf x = \Delta \bm X + \mathbf d (\bm X + \Delta \bm X, t) - \mathbf d (\mathbf X, t) $$

$\Delta \bm X$가 충분히 작아 $\mathbf d (\bm X + \Delta \bm X, t)$를 선형으로 근사할 수 있다면 다음과 같다.
$$ \begin{aligned} & \Delta x_i = \Delta \bm X_i + \frac{\partial d_i}{\partial \bm X_j} \Delta \bm X \\ \Leftrightarrow \enspace & \Delta \mathbf x = \Delta \bm X + \nabla \mathbf d \cdot \Delta \bm X \\ \Leftrightarrow \enspace & \Delta \mathbf x = \mathbf F \Delta \bm X   \end{aligned} $$

이 때, 2차 텐서인 $\Delta \mathbf d$를 `displacement gradient`, $\mathbf F$를 `deformation gradient`라고 한다. $\bf F$를 deformation gradient라고 하는 이유는 $\bf F = \nabla p$이기 때문이다.

### 길이 변화
다음으로, 두 점으로 이루어진 요소의 길이가 변형 후 어떻게 변하는지 보자.
$$ \Delta \mathbf x \cdot \Delta \mathbf x = \Delta \bm X ^T \mathbf F^T \mathbf F \Delta \bm X = \Delta \bm X ^T \mathbf C \Delta \bm X $$
...
이를 대입하면 다음과 같다.
$$ \begin{equation} \begin{gathered} \begin{aligned} & \Delta \mathbf x \cdot \Delta \mathbf x = \Delta \bm X^T \Delta \bm X + 2\mathbf \Delta \bm X^T \mathbf E_L \Delta \bm X \\ \Leftrightarrow \enspace & (\Delta s)^2 = (\Delta S)^2 + 2 (\Delta S)^2 (\bf n \cdot E_L n)  \end{aligned} \\ \text{Where, } \lVert \Delta \mathbf x \rVert_2 = \Delta s, \quad  \Delta \bm X = \Delta S \mathbf n \end{gathered} \end{equation}  $$

만약 $\bf d$가 모든 점에서 동일한 강체 운동일 경우 $\nabla \bf d = 0$이고 $\mathbf E_L = 0$임으로 $\bf C = I$가 된다. 즉, $\mathbf E_L$은 변위의 변화율, 변형률을 표현하는 값이 된다. 이 때, $\mathbf E_L$을 `Lagrange strain tensor` 또는 `finite deformation tensor`라고 한다.

### 각도 변화
이번에는, 수직인 요소들이 이루는 각도가 변형 후 어떻게 변하는지 보자.
...
두 점 $\bm X, \bm X + \Delta \bm X$이 이루는 요소를 $l_1$ 두 점 $\bm X, \bm X + \Delta \bm Y$가 이루는 요소를 $l_2$라 할 떄, 변형후 $l_1$과 $l_2$가 이루는 각도를 생각해보자.

$$ \begin{equation} \begin{gathered} \begin{aligned} & \Delta \mathbf x \cdot \Delta \mathbf y = \Delta \bm X^T \Delta \bm Y + 2\mathbf \Delta \bm X^T \mathbf E_L \Delta \bm Y \\ \Leftrightarrow \enspace & \Delta s_1 \Delta s_2 \cos \theta = \Delta S_1 \Delta S_2 (\bf n \cdot E_L m)  \\ \Leftrightarrow \enspace & \cos \theta = \frac{\Delta S_1 \Delta S_2 (\bf n \cdot E_L m)}{\Delta s_1 \Delta s_2 } \end{aligned} \\ \text{Where, } \lVert \Delta \mathbf x \rVert_2 = \Delta s_1, \quad \lVert \Delta \mathbf y \rVert_2 = \Delta s_2 \end{gathered} \end{equation}  $$
...
#### 길이 변화
변형률이 매우 작기 때문에 $\Delta s \approx \Delta S$라 하고 식(3)을 변형하면 다음과 같다.
$$ \begin{aligned} & (\Delta s)^2 = (\Delta S)^2 + 2 (\Delta S)^2 (\bf n \cdot E \cdot n) \\ \Leftrightarrow \enspace & \frac{(\Delta s)^2 - (\Delta S)^2}{2 (\Delta S)^2} = \bf n \cdot En \\ \Leftrightarrow \enspace & \frac{\Delta s - \Delta S}{\Delta S} = \bf n \cdot En \end{aligned} $$

$\bf n$방향에 있던 요소의 단위 길이당 변화율은 $\bf n \cdot En$으로 나타나며 $\bf e_1,e_2,e_3$방향의 단위 길이당 변화율은 각 각 $E_{11},E_{22},E_{33}$로 나타난다. 따라서 $\bf E$의 대각성분에 있는 값들을 `normal strain`이라고 한다.

#### 각도 변화
변형률이 매우 작기 때문에 $\Delta s_i \approx \Delta S_i$라 하고 $\theta = \frac{\pi}{2} - \gamma$, $\gamma \approx 0$로 두고 식(4)을 변형하면 다음과 같다.
$$ \begin{aligned} & \cos \left( \frac{\pi}{2} - \gamma \right) = 2 (\mathbf n \cdot \mathbf{Em}) \\ \Leftrightarrow \enspace & \sin\gamma = 2 (\mathbf n \cdot \mathbf{Em}) \\ \Leftrightarrow \enspace & \gamma = 2 (\mathbf n \cdot \mathbf{Em}) \end{aligned}  $$

$\bf n, m$이 기본 기저일 경우 $\gamma = 2E_{ij}$가 되며 이는 기존에 $x_i, x_j$방향에 있던 두요소가 이루던 각도의 변화량이다.


> 참고  
> [book] (Lai et al) Introduction to Continuum Mechanics Chapter3.7, 3.8
