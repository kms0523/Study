...
위 식을 보면, $\nabla \mathbf d \Delta \bm X$이 변화량을 나타내는 항이며, $\mathbf F$에 의해서 $\Delta \bm X$가 $\Delta \mathbf x$로 변한되고 있다. 이 때, 2차 텐서인 $\Delta \mathbf d$를 `displacement gradient`, $\mathbf F$를 `deformation gradient`라고 한다. $\bf F$를 deformation gradient라고 하는 이유는 $\mathbf F = \nabla \mathbf p$이기 때문이다.
...
$$ \mathbf W \Delta \mathbf x = \boldsymbol \omega \times \Delta \mathbf x $$

따라서, 요소의 시간변화율을 $\mathbf D, \mathbf W$로 나타내면 다음과 같다.
$$ \frac{D}{Dt} \Delta \mathbf x = \mathbf D \Delta \mathbf x + \mathbf W \Delta \mathbf x = \mathbf D \Delta \mathbf x + \boldsymbol \omega \times \mathbf x$$
...
식 (9)에서 알 수 있듯이, $\mathbf W$는 $\Delta \mathbf x$로 표현되는 요소의 길이 변화에 영향을 주지 않는다.

이 떄, $\mathbf w$는 spin tensor의 dual vector이며 `angular velcoity`라고 한다.
