...
## Deformation
두 점 $\bm X, \bm X + \Delta \bm X$와 변위 $\mathbf d(\bm X, t)$가 있을 때, $t$일 때 두 점의 공간좌표는 다음과 같다.
...
### 길이 변화
다음으로, 두 점으로 이루어진 요소의 길이가 변형되는지 보자.
...
$\Delta \bm X$가 충분히 작을 때, $\Delta \mathbf x$는 다음과 같다.
...
$\Delta \bm X, \Delta \bm Y$가 충분히 작을 때, $\Delta \mathbf x, \Delta \mathbf y$는 다음과 같다.
