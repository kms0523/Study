...
물체에 하중이 가해졌을 때, 물체 내부의 분자들에 작용하는 힘을 표시한 (a)와 물체 내부를 가상의 평면을로 나눈 (b)를 보자.

(b)에 나타난 가상의 평면에 작용하는 힘을 정확히 표현하기 위해서는 평면 위의 분자들에 작용하는 힘을 모두 고려해야 한다. 하지만 분자의 개수가 너무 많기 때문에 이는 매우 어려운 일이였다. 
