...
식(8)을 간단히 하기위해 다음과 같이 가정하자.
1. 하중과 변형은 선형관계이다
2. 하중은 정적으로 가해진다.
3. 하중을 제거하면 변형이 완전히 사라진다.
4. 변형이 매우 작다

위의 가정을 통해 식(8)을 다음과 같이 표현할 수 있다.
...
# 등방성 선형 탄성 재료
특정 방향에 따라 물성치가 변하지 않을 때 `등방성(isotropic)`재료라고 한다. 만약 방향에 따라 물성치가 변할 경우 이를 `비등방성(anistropic)`재료라고 한다.

등방성 재료의 경우 모든 방향에서 응력과 변형률 관계가 같아야 한다. 따라서 등방성 선형 탄성 재료의 경우 $\bf C$가 isotropic tensor여야 한다.

fourth-order tensor의 경우 일반적으로 다음과 같이 표현 가능하다.
$$ C_{ijkl} = \lambda \delta_{ij}\delta_{kl} + \alpha \delta_{ik}\delta_{jl} + \beta \delta_{il}\delta_{jk} $$

식(9)에 대입하여 정리하면 다음과 같다.
$$ \sigma_{ij} = \lambda \epsilon_{kk}\delta_{ij} + (\alpha + \beta)\epsilon_{ij} $$

$\alpha + \beta = 2\mu$라 두면 다음과 같다.
$$ \begin{equation} \sigma_{ij} = \lambda \epsilon_{kk}\delta_{ij} + 2\mu\epsilon_{ij} \end{equation} $$

식 (11)로부터 다음 관계식을 얻을 수 있다.
$$ \sigma_{ii} = (3\lambda + 2\mu) \epsilon_{kk} $$

따라서 식(11)을 변형률에 대해 정리하면 다음과 같다.
$$ \begin{equation} \epsilon_{ij} = \frac{1}{2\mu} \left( \sigma_{ij} - \frac{\lambda}{3\lambda + 2\mu}\sigma_{kk}\delta_{ij} \right) \end{equation} $$

식(12)과 식(13)에서 공통적으로 나타나는 두 개의 재료 상수 $\lambda$, $\mu$를 `Lame's constant`라고 한다. $\epsilon$이 무차원 변수임으로 Lame's constant는 응력과 동일한 차원을 갖는다.

> 참고  
[book] (Lai et al) Introduction to Continuum Mechanics Chap5.3

## unaxial stress state
만약 하나의 normal stree만 존재하고 나머지는 전부 0인 경우를 `uniaxial stress state`라고 한다. 이제 식(13)를 사용해서 uniaxial stress state를 분석해보자.

식(13)에 uniaxial stress state를 대입하면 다음과 같다.
...
식(14)로부터 새로운 재료상수를 얻을 수 있다.
$$ \begin{equation} \begin{gathered} \frac{\sigma_{11}}{\epsilon_{11}} =  \frac{\mu (3\lambda + 2\mu)}{\lambda + \mu} = E_Y \\ \frac{\epsilon_{22}}{\epsilon_{11}} = \frac{\epsilon_{22}}{\epsilon_{11}} = - \frac{\lambda}{2(\lambda + \mu)} = -\nu \end{gathered} \end{equation}   $$

이 때, $E_Y$를 `탄성계수(Young's modulus, modulus of elasticity)`라고 하며 $\nu$를 `프와송비(Poisson's ratio)`라고 한다.

식(15)에서 $\lambda$를 제거하면 $\mu(E_Y,\nu)$을 얻을 수 있고 이를 통해 $\lambda(E_Y,\nu)$도 얻을 수 있다.
$$ \begin{equation} \begin{aligned} \mu &= \frac{E_Y}{2(1+\nu)} \\ \lambda &= \frac{\nu}{(1-2\nu)(1+\nu)}E_Y \end{aligned} \end{equation} $$

식(15)에서 얻은 재료 상수를 식(13)에 대입하고 식(16)의 관계를 이용하면 다음과 같은 변형률-응력 관계식을 얻는다.
...
simple shear stress state일 때 식(13)은 다음과 같다.
$$ \begin{equation} \epsilon_{12} = \frac{\sigma_{12}}{2\mu} \end{equation} $$

식(18)로부터 새로운 재료상수를 얻을 수 있다.
$$ \frac{\sigma_{12}}{2\epsilon_{12}} = \mu = G $$

이 때, $G$를 `전단계수(shear modulus)`라고 한다.

다음으로, simple shear stress state일 때 식(17)은 다음과 같다.
$$ \begin{equation} \epsilon_{12} = \frac{1+\nu}{E_Y}\sigma_{12} \end{equation} $$

따라서 다음이 성립한다.
$$ \frac{\sigma_{12}}{2\epsilon_{12}} = \frac{E_Y}{2(1 + \nu)}=G $$
...
hydrostatic state of stress일 때 식(13)은 다음과 같다.
$$ \begin{equation} \epsilon_{ii} = \frac{3p}{3\lambda + 2\mu} \end{equation} $$

식(20)으로부터 다음과 같은 재료상수를 얻을 수 있다.
$$ \frac{p}{\epsilon_{ii}} = \lambda + \frac{2}{3}\mu = k $$

이 떄, $k$를 `체적계수(bulk modulus)`라고 한다.

## 참고
Lame's constant를 시작으로 탄성 계수, 프와송 비, 전단계수, 체적계수등이 있는데, 등방성 선형 탄성재료에서는 이 중 2개의 재료상수만 독립이다. 두개의 독립적인 재료상수가 주어졌을 때 나머지 재료상수들이 어떻게 표현되는 지는 [book] (Lai et al) Introduction to Continuum Mechanics의 Table 5.1(212p)에 잘 정리되어 있다.
