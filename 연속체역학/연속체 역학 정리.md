...
## 참고 
[book] (Lai et al) Introduction to Continuum Mechanics Chapter4  
[note] (kelly) An Introduction to Solid Mechanics Chapter 3.3
...
식(11)과 식(12)에서 공통적으로 나타나는 두 개의 재료 상수 $\lambda$, $\mu$를 Lame's constant라고 한다. $\epsilon$이 무차원 변수임으로 Lame's constant는 응력과 동일한 차원을 갖는다.

## unaxial stress state
만약 하나의 normal stree만 존재하고 나머지는 전부 0인 경우를 `uniaxial stress state`라고 한다. 이제 식(12)를 사용해서 uniaxial stress state를 분석해보자.

식(12)에 uniaxial stress state를 대입하면 다음과 같다.
