...
## Deformation
두 점 $\bm X, \bm X + \Delta \bm X$와 변위 $\mathbf d(\bm X, t)$가 있을 때, 시간에 따른 두 점의 공간좌표는 다음과 같다.
...
이 때, 2차 텐서인 $\Delta \mathbf d$를 `displacement gradient`, $\mathbf F$를 `deformation gradient`라고 한다. $\bf F$를 deformation gradient라고 하는 이유는 $\mathbf F = \nabla \mathbf p$이기 때문이다.
...
두 점 $\bm X, \bm X + \Delta \bm X$으로 이루어진 요소를 생각해보자. 시간 $t$일 때, 요소는 다음과 같이 변형된다.
...
$$ \begin{aligned} \frac{D}{Dt} \Delta \mathbf x &= (\nabla_{\bm X} \mathbf u) \Delta \bm X \\ &= (\nabla_{\mathbf x} \mathbf v) \Delta \mathbf x \end{aligned} $$
...
$\nabla \mathbf v$도 다음과 같이 symmetric part와 antisymmetric part로 나눌 수 있다.
...
#### 길이의 시간 변화율
이번에는 시간에 따라 길이가 어떻게 변화하는지 알아보자.

시간 $t$에서 요소가 다음과 같이 표현된다고 하자.
$$ \Delta \mathbf x = \Delta S \mathbf n $$

따라서 시간 $t$에서 요소의 길이는 다음과 같다.
$$ \begin{equation} \Delta \mathbf x \cdot \Delta \mathbf x = (\Delta S)^2 \end{equation} $$

시간에 따른 변화율을 보기위해 식(7)을 물질미분 하면 다음과 같다.
$$ \begin{equation} 2 \Delta \mathbf x \cdot \frac{D}{Dt} \Delta \mathbf x = 2 \Delta S \frac{D}{Dt}\Delta S \end{equation} $$

식(8)의 왼쪽항을 rate of deformation tensor와 spin tensor를 이용해서 표현하면 다음과 같다.
$$ \Delta \mathbf x \cdot \frac{D}{Dt} \Delta \mathbf x = \Delta \mathbf x \cdot \nabla \mathbf v \Delta \mathbf x = \Delta \mathbf x \cdot (\mathbf D + \mathbf W) \Delta \mathbf x = \Delta \mathbf x \cdot \mathbf D \Delta \mathbf x $$

다시 식(8)에 대입해서 정리하면 다음과 같다.
$$ \frac{1}{\Delta S} \frac{D}{Dt} \Delta S = \mathbf n \cdot \mathbf{Dn} $$
