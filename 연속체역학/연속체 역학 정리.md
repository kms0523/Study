...
두 점 $\bm X, \bm X + \Delta \bm X$으로 이루어진 요소를 생각해보자. 시간 $t$에서 요소는 다음과 같이 변형된다.
$$ \begin{equation} \Delta \mathbf x = \mathbf x(\bm X + \Delta \bm X, t) - \mathbf x(\bm X, t) \end{equation}  $$

식(6)을 물질미분한 뒤, 속도를 물질관점과 공간관점에서 나타내면 다음과 같다.
...
#### 길이변화율
$$ \Delta \mathbf x \cdot \Delta \mathbf x = (\Delta S)^2$$

$$ 2 \Delta \mathbf x \cdot \frac{D}{Dt} \Delta \mathbf x = 2 \Delta S \frac{D}{Dt}\Delta S $$
