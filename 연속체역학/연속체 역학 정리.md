...
연속체의 운동은 공간좌표로 표현될 수 있음으로, 운동을 표현하기 위해 연속체 내부의 모든점의 공간좌표를 벡터 함수로 나타내면 다음과 같다.
$$ \mathbf x_n = \mathbf x_n(t), \quad n=1, \cdots, N, \cdots $$
...
물질 좌표를 이용해서 각 점의 공간좌표을 벡터 함수로 나타내면 다음과 같다.
$$ \mathbf x = \mathbf x(\bm X,t) $$
