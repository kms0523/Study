...
x = sympy.symbols("x")
exact_solution = 0.5 * (1 + sympy.exp(-x)*(sympy.cos(x) - sympy.sin(x)))

plt1 = plot(exact_solution, (x, 0, sympy.pi/2), legend=True,label='$u_{ex}$', show = False)

def B_WRF(w,u):    
    dudx = diff(u, x)
    d2udx2 = diff(u, x, 2)
    return integ(w*exp(x)*(dudx + d2udx2), (x, 0, sympy.pi/2))

def l_WRF(w):
    return integ(w*sympy.sin(x), (x, 0,sympy.pi/2))

def B_WF(w,u):
    dwdx = diff(w,x)
    dudx = diff(u,x)

    wdudx0 = (-sympy.exp(x)*w*dudx).subs(x,0)

    return -integ(sympy.exp(x)*dwdx*dudx, (x, 0,sympy.pi/2)) - wdudx0
...
print(solution_WRF)
print(solution_WF)
print(diff(solution_WRF,x).subs(x,sympy.pi/2).evalf())
print(diff(solution_WF,x).subs(x,sympy.pi/2).evalf())

plt2 = plot(solution_WRF, (x,0,sympy.pi/2), legend=True, label='$u_{WRF}$',show = False)
plt3 = plot(solution_WF, (x,0,sympy.pi/2), legend=True, label='$u_{WF}$',show = False)
