...
std::vector<std::string_view> parse(std::string_view sv,
	...
	DB(const std::string& TN, const std::vector<std::vector<std::string>>& clinets_info_keys, const std::vector<std::vector<std::string>>& clinets_info_vals) {
		...
		for (const auto& clinet_infos : clinets_info_keys) {
			for (const auto& info : clinet_infos) {
				this->key_to_var_id_.emplace(std::string_view(info.data()),
						initial_val);
			}
		}
...
public:
	...
	explore_query(std::string_view query) const {
		if (query.find(this->table_name_) == std::string_view::npos) {
			return {};
		}
...
private:
	std::vector<int> make_valid_client_ids(std::string_view target_key,
			std::string_view operation,
			std::string_view criteria) const {
		const auto num_clients = this->client_id_var_id_to_vals_.size();
...
std::vector<std::vector<std::string>>
solution(std::string TN, std::vector<std::vector<std::string>> P,
		std::vector<std::vector<std::string>> V, std::string Q) {
	DB db(TN, P, V);
	db.update_query(Q);
	return db.print();
}
...
std::set<std::string_view> make_infokey_set(
		const std::vector<std::vector<std::string>>& clinets_info_keys) {
...
std::string make_query(const std::string_view TN,
		const std::set<std::string_view>& info_key_set) {
	auto info_size = 0;
...
std::vector<std::string> make_insert_query(
		const std::string_view TN,
		const std::vector<std::set<Information>>& clients_keyval_set) {
...
std::vector<std::set<Information>> make_clients_information_set(
		const std::vector<std::vector<std::string>>& clinets_info_keys,
		const std::vector<std::vector<std::string>>& clinets_info_vals) {
	const auto num_clients = clinets_info_keys.size();
	...
	for (int i = 0; i < num_clients; ++i) {
		...
		for (int j = 0; j < num_key; ++j) {
			information_set.insert(
					Information(client_info_keys[j], client_info_vals[j]));
		}
...
std::vector<std::string> solution2(std::string TN,
...
std::vector<std::vector<std::string>>
solution3(std::string TN, std::vector<std::vector<std::string>> P,
		std::vector<std::vector<std::string>> V) {
	DB db(TN, P, V);
	return db.print();
}
...
std::vector<std::vector<std::string>>
solution4(std::string TN, std::vector<std::vector<std::string>> P,
		std::vector<std::vector<std::string>> V, std::string Q) {
	DB db(TN, P, V);
