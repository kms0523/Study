...
new_row = mat(zeros((1,n-1)))
for i in range(0,n-1):
	new_row[0,i] = u[i+1].subs(x,2)

B_BC = np.vstack([new_row, B_BC])
l_BC = np.vstack([[5],l_BC])

# remove 1,2eq
row = 0
B1 = np.delete(B_BC,1,row)
l1 = np.delete(l_BC,1,row)
B2 = np.delete(B1,1,row)
l2 = np.delete(l1,1,row)
a2 = inv(B2) * l2
...
# remove 2,3eq
row = 0
B1 = np.delete(B_BC,2,row)
l1 = np.delete(l_BC,2,row)
B2 = np.delete(B1,2,row)
l2 = np.delete(l1,2,row)
a2 = inv(B2) * l2
