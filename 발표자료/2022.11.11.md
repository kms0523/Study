...
$$ \phi(\sigma, \alpha, \epsilon_e^p) = \sqrt{\frac{3}{2}} \lVert\eta\rVert - K(\epsilon_e^p) $$

$$ \text{Where, } \eta(\sigma, \alpha) = \tilde\sigma - \alpha $$

$K$는 isotropic hardening modulus이다.

## J2 plasticity model
명제1,2에 의해 다음이 성립한다.
$$ \begin{aligned} \frac{\partial \epsilon^p}{\partial t} &= \sqrt{\frac{3}{2}} \gamma N \\ \frac{\partial\alpha}{\partial t} &= \sqrt{\frac{3}{2}}H(\epsilon_e^P)\gamma N \\ \frac{\partial \epsilon_e^p}{\partial t} &= \gamma \end{aligned} $$
...
$$ \begin{aligned} \frac{\partial \epsilon^p}{\partial t} &= \gamma\frac{\partial\phi}{\partial\sigma} \\&= \gamma\frac{\partial\phi}{\partial\eta}\frac{\partial\eta}{\partial\tilde\sigma}\frac{\partial\tilde\sigma}{\partial\sigma} \\&= \sqrt{\frac{3}{2}}\gamma N : I : I_{dev}\\&= \sqrt{\frac{3}{2}}\gamma N \quad\tiny\blacksquare \end{aligned} $$

### 명제2
다음을 증명하여라.
$$ \frac{\partial\alpha}{\partial t} = \sqrt{\frac{3}{2}}H\gamma N $$

**Proof**
J2 plasticity model의 hardening rule과 명제1에 의해 다음이 성립한다.
$$ \begin{aligned} \frac{\partial\alpha}{\partial t} &= H\frac{\partial\epsilon^p}{\partial t} \\&= \sqrt{\frac{3}{2}}H\gamma N \end{aligned} $$
...
$\Delta \epsilon^p \neq 0$일 때, 다음을 증명하여라.
$$ \alpha^{k+1} = \alpha^k + \sqrt{\frac{3}{2}}\Delta\gamma H^{k+1}N^{k+1} $$
...
$$ \begin{aligned} \alpha^{k+1} &= \alpha^k + \Delta t \sqrt{\frac{3}{2}}\gamma H^{k+1}N^{k+1} \\&= \alpha^k + \sqrt{\frac{3}{2}}\Delta\gamma H^{k+1}N^{k+1} \quad\tiny\blacksquare \end{aligned} $$
...
$$ \begin{aligned} & f({\sigma}^{k+1}, \alpha^{k+1}, (\epsilon_e^p)^{k+1} ) = 0 \\\Rightarrow\enspace& \sqrt{\frac{3}{2}}\lVert\eta^{k+1}\rVert - K^{k+1} \\\Rightarrow\enspace& \sqrt{\frac{3}{2}} \bigg\lVert\eta^{tr} - \sqrt{6}\mu\Delta\gamma N^{k+1} + \sqrt{\frac{3}{2}}\Delta\gamma H^{k+1}N^{k+1} \bigg\rVert  - K^{k+1} = 0 \\\Rightarrow\enspace& \sqrt{\frac{3}{2}} \lVert\eta^{tr}\rVert + (\frac{3}{2}H^{k+1} - 3\mu)\Delta\gamma - K^{k+1} = 0 \end{aligned} $$
