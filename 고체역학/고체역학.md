...
# 변형률
`변위(displacement)`는 물체의 기존 위치와 바뀐 위치의 차이이다.
...
위 그림과 같이 변형되었다고 할 때, 1차원 변형률에 정의를 그대로 확장한 2차원 변형률은 다음과 같다.
...
만약 변위가 충분히 작아 $\frac{\partial d_i}{\partial x_j} \ll 1$라 가정하면, x축 변형률은 다음과 같이 근사 할 수 있다.
$$ \epsilon_{11} \approx \frac{\partial d_1}{\partial x} $$

위와 동일한 과정을 y축 변형률에 적용하면 다음과 같다.
...
즉, 미소 변위 이론은 변형이 충분히 작아  변형률을 다음과 같이 근사할 수 있다는 가정이다.
