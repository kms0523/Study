...
# 변형률
물체가 책상위에 존재하고 그 책상이 움직이면 물체의 입자들은 강체와 같이 공간상에서 움직이고 `변위(displacement)`를 갖게 된다. 반면에 외력의 작용을 받게 되면 그 내부에서는 응력이 발생함과 동시에, 강체가 아닌 이상 그 물체를 구성하는 각 분자와 분자 상호간의 운동으로 인하여 물체의 상태가 변하게 되어 형태와 크기가 달라지는 `변형(deformation)`을 겪게 된다. 이러한 변형은 `변형률(strain)`로써 설명한다.

`운동학(kinematics)`은 변위나 변형에 대해 연구하는 학문이다.
...
이는 현재 길이에서 미소한 변형률을 합한다는 의미를 갖으며 변형된 순간순간의 현재 길이를 반영한것이다.

진 변형률은 공학 변형률에 비해 변형률을 표현하는데 장점이 있다.
...
따라서 충분히 작은 $\epsilon$에 대해서는 $\epsilon_t \approx \epsilon$이 성립한다. 즉 변형이 작을 때에는 진변형률과 공학변형률이 비슷한 값을 가지게 된다.
...
### exact strain
이번에는 exact strain의 정의를 살펴보자.

1. $x_1$축의 normal strain $\epsilon_{11}$은 초기에 $x_1$축과 평행한 직선이 단위길이당 늘어난 길이이다.
2. $x_2$축의 normal strain $\epsilon_{22}$은 초기에 $x_2$축과 평행한 직선이 단위길이당 늘어난 길이이다.
3. 전단 변형률 $\epsilon_{12} = \epsilon_{21}$은 초기에 $x_1$축과 평행한 직선이 변형된 각도와 $x_2$축과 평핸한 직선이 변형된 각도의 평균값이다.

<p align="center">
<img src="./image/strain3.png" width= "400" height="200">
</p>

exact strain의 정의에 따른 변형률은 다음과 같다.
$$ \epsilon_{11} = \frac{A'B' - AB}{AB}, \quad \epsilon_{22} = \frac{A'C' - AC}{AC}, \quad \epsilon_{12} = \epsilon_{21} = \frac{1}{2} (\theta + \lambda) $$

여기서도 알 수 있듯이, 점 $A$가 강체운동에 의해 변형이 되더라도 변형률은 변하지 않는다.

### small strain assumption
만약 변형이 충분히 작다면 변형률을 다음과 같이 근사할 수 있다.
...
이러한 근사를 공학변형률(engineering strain), small strain 또는 infinitesimal strain이라고 하며 변형이 작을 때 성립한다.

### 공학전단변형률
`공학적전단변형률(engineering shear strain)` $\gamma_{12}$는 위의 정의한 `텐서전단전형률(tensorial shear strain)` $\epsilon_{12}$와 다음 관계를 만족한다.
...
# 미소변위 이론
연속체 역학에서 `미소변위 이론(infinitesimal strain theory)`이란 실제 물체에 비해 변위가 충분히 작아 기하학적인 형상과 밀도와 강성같은 물질의 구성적인 특징이 변형에 의해 변하지 않는다고 가정한 뒤 고체의 변형에 대해 수학적으로 기술한 이론이다.

이 이론을 판(plate), 쉘(shell)과 같이 얇고 변형이 쉬운 물체에 적용할 경우 부정확한 결과가 나올 수 있다.

## 미소 변형률 텐서
### 유한변위 이론에 의한 유도
`유한변위 이론(finite strain theory)`에서 유도한 유한변형 텐서는 다음과 같다.
...
따라서 `미소 변형률 텐서(infinitesimal strain tensor, Cauchy's strain tensor, linear strain tensor)` $\boldsymbol{\epsilon}$는 다음과 같이 간단하게 표현할 수 있다.
...
### 기하학적인 유도
육면체 미소요소에 응력이 작용하면 육면체 미소요소는 변형을 하게 된다. 이 변형으로 발생하는 `변위(displacement)`를 $\bf d = d(x)$라 하자.

미소변위 이론의 가정에 의해 고차항을 무시할 수 있음으로 변위를 선형근사한 뒤 변형률성분을 계산하면 다음과 같다.

$$ \begin{align} \epsilon_{11} & = \frac{\partial d_1}{\partial x_1} \\ \epsilon_{22} & = \frac{\partial d_2}{\partial x_2} \\ \epsilon_{33} & = \frac{\partial d_3}{\partial x_3} \\ \gamma_{12} = \gamma_{21} & = \frac{\partial d_1}{\partial x_2} + \frac{\partial d_2}{\partial x_1} \\ \gamma_{23} = \gamma_{32} & = \frac{\partial d_2}{\partial x_3} + \frac{\partial d_3}{\partial x_2} \\ \gamma_{31} = \gamma_{13} & = \frac{\partial d_3}{\partial x_1} + \frac{\partial d_1}{\partial x_3} \end{align} $$

즉, 변위 $\bf d$를 통해 3개의 수직변형률성분과 3개의 전단변형률성분을 모두 표현할 수 있다.

이러한 공학적인 변형률 정의를 사용하여 미소 변형률 텐서 $\boldsymbol{\epsilon}$를 표현하면 다음과 같다.
$$ \epsilon_{ij} = \begin{cases} \epsilon_{ij} & i = j \\\\ \displaystyle\frac{1}{2}\gamma_{ij} & i \neq j \end{cases} $$

## 공학적 변형률 벡터
미소 변형률 텐서를 보면 3개의 수직변형률성분과 3개의 전단변형률성분 총 6개의 독립적인 성분을 가지고 있는 것을 알 수 있다. 이를 벡터 형태로 나타내면 다음과 같다.

$$ \frak{e} = \begin{bmatrix} \epsilon_{11} \\ \epsilon_{22} \\ \epsilon_{33} \\ \gamma_{12} \\ \gamma_{23} \\ \gamma_{31}  \end{bmatrix} $$

## 참고
[Infinitisimal strain theory - Wiki](https://en.wikipedia.org/wiki/Infinitesimal_strain_theory#Infinitesimal_strain_tensor)
