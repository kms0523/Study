...
# 변형률
어떤 물체이든 외력의 작용을 받게 되면 그 내부에서는 응력이 발생함과 동시에, 강체가 아닌 이상 그 물체를 구성하는 각 분자와 분자 상호간의 운동으로 인하여 물체의 상태가 변하게 되어 형태와 크기가 달라지게 된다. 물체에 발생하는 변형량은 물체의 크기에 따라 차이가 있으므로 응력이 같다고 하더라도 크기가 큰 물체는 변화량 역시 크게 된다. 

변형의 정도를 비교하기 위해서는 외력의 작용으로 인하여 발상된 변형량과 원래 치수와의 비, 즉 단위 길이에 대한 변형량을 구함으로써 물체의 크기에 상관하지 않고 그 물체의 변형의 정도를 알 수 있다.

`변형률(strain)`은 변형량과 원래 치수와의 비를 의미한다.
