...
좌표변환 행렬을 $T$라고 두면 바뀐 좌표에서의 선형변환은 다음과 같이 표현된다.
$$ \boldsymbol{\epsilon'} = T^{-1} \boldsymbol{\epsilon}T $$

만약 $T$가 회전행렬 $R$이라면 $R$은 orthogonal matrix임으로 다음이 성립한다.
$$ \boldsymbol{\epsilon'} = R^{T} \boldsymbol{\epsilon}R $$
